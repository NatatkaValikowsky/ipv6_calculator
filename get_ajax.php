...
function get_subnets($adress, $prefix, $count) {
    ...
    $subnet_id_length = get_nearest_pow($count);
    ...
    for($i = 1; $i <= $count; $i++) {
        ...
        echo('<tr><td>' . $ip . '</td><td>' . ($prefix+$subnet_prefix) . '</td><td>' . get_network_range($ip,$prefix+$subnet_prefix) . '</td></tr>');
    }
}
...
<?php
	if($_POST['second_area'] <= bcpow(2,128-$_POST['select_list'])) {
		echo('<table border=0>');
		echo('<tr><td>IP-адрес подсети</td><td>Префикс подсети</td><td>Диапазон адресов</td></tr>');
